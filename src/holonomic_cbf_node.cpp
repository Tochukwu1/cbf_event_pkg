...
private:
    ...
    void trigger_loop()
    {
        ...
        // 1. Calculate the standard nominal safety boundary
        double b = -alpha_fun(h) - Lfh_val;
        
        // 2. Calculate the safety residual r(x,u)
        double r = b - Lgh_val.dot(u_held_); 
        
        // 3. Define the Robust Jump Map constraint (tightened by epsilon)
        double b_tight = b + epsilon_;

        double update_thresh = std::min(0.05, 0.2 * dist_to_goal);
        double nom_error = (u_held_ - u_nom).norm();

        // 4. Exact Boundary Detection Trigger (Triggers at exactly 0)
        bool is_cbf_trigger = (r >= 0.0);
        ...
        RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
            ...
            " Residual r: %.3f (Triggers CBF if >= 0.000)\n"
            " Nom Error : %.3f (Triggers Update if > %.3f)\n"
            " u_held    : [%.3f, %.3f]",
            mode_to_string(current_mode_).c_str(), dist_to_goal, h, 
            r, nom_error, update_thresh, u_held_(0), u_held_(1));

        if (is_cbf_trigger || is_nominal_update || is_startup)
        {
            // 5. Solve the QP using the tightened boundary constraints
            Eigen::Vector2d u_safe = solve_cbf_qp(q_current_, u_nom, b_tight, Lgh_val);
            u_held_ = u_safe; 

            // Track state transitions based on the tightened condition
            bool is_cbf_intervening = (Lgh_val.dot(u_nom) < b_tight);

            if (is_cbf_intervening)
            {
                if (current_mode_ != ControlMode::CBF_ACTIVE) {
                    RCLCPP_WARN(this->get_logger(), ">> SAFETY BOUNDARY REACHED! Executing Robust Jump Map.");
